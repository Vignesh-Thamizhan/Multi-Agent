...
## 🌟 Featured: Local Inference Mode (Ollama)

NeuralForge now supports **100% Local Inference**. You can run the entire multi-agent pipeline on your own machine without sending any data to external APIs.

- **Private & Secure**: No API keys required. All data stays on your machine.
- **Cost-Free**: Run complex multi-agent chains as much as you want.
- **Ollama Integration**: Seamlessly connects to your local Ollama instance.
- **Auto-Summarized Code Space**: Since local models may have limited tool support, NeuralForge automatically extracts and summarizes all generated code blocks into a dedicated **Code Space** at the end of the pipeline.

---

## 🆕 What's New

- **Local Inference Mode**: Run the entire pipeline locally using Ollama (qwen3.5:4b).
- **Ollama Code Space**: A dedicated UI area that parses and displays all code generated by local models.
- **Debugger Agent**: RAG-powered agent that autonomously analyzes code and finds bugs.
- **Parallel Pipeline**: Execution mode for faster iteration across multiple agents.
- **MCP File Tools**: Agents can autonomously manage files in your session workspace.
- **Vision Support**: Processes images and PDFs via Gemini 1.5 Flash.
 per-session with retrieval APIs
...
To run NeuralForge in local mode:
...
4. **Switch Mode in UI**: Open the NeuralForge web app and select **Local** mode from the top bar.
5. **No API keys needed**: In local mode, the system will bypass cloud providers and route all agent traffic to your local instance.
